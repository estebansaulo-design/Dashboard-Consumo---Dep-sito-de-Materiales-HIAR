...
const totalPartida = 239650505.10;
...
function getReading(monthData) {
  if (monthData.ejecutado >= 130) return { label: 'Muy por encima del PDG', tone: 'bg-red-600 text-white' };
  if (monthData.ejecutado >= 100) return { label: 'Por encima del PDG', tone: 'bg-amber-500 text-black' };
  return { label: 'Dentro del PDG', tone: 'bg-emerald-600 text-white' };
}
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-sm font-medium text-slate-500">Depósito de Materiales SJ</div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Dashboard del ejercicio económico</h1>
            <p className="mt-1 text-slate-600">Visualización ejecutiva para contrastar PDG, consumo real y ranking de egresos</p>
          </div>
          <div className="flex w-full flex-col gap-2 md:w-[240px]">
            <div className="text-sm font-medium text-slate-600">Elegir mes</div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-slate-500">Mes seleccionado</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-1 text-sm text-slate-600">Consumo del mes: {fmtMoney(currentMonth.consumo)}</div>
            </CardContent>
          </Card>

          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-slate-500">PDG del mes</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{fmtMoney(currentMonth.pdg)}</div>
              <div className="mt-1 text-sm text-slate-600">Ejecución: {fmtPct(currentMonth.ejecutado)}</div>
            </CardContent>
          </Card>

          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-slate-500">Acumulado al mes</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{fmtMoney(consumedToDate)}</div>
              <div className="mt-1 text-sm text-slate-600">PDG acumulado: {fmtMoney(pdgToDate)}</div>
            </CardContent>
          </Card>

          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-slate-500">Lectura rápida</CardTitle>
            </CardHeader>
            <CardContent>
              <Badge className={`${reading.tone} mb-2 w-fit`}>{reading.label}</Badge>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Avance del ejercicio</CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <div>
                ...
                <Progress value={Math.min(pdgProgress, 100)} className="h-4" />
                <div className="mt-2 text-xs text-slate-500">Comparación contra la partida anual total: {fmtMoney(totalPartida)}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Qué mirar en {currentMonth.fullMonth}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm leading-6 text-slate-700">
              <p>{currentBreakdown?.summary || 'Sin lectura ejecutiva cargada para este mes.'}</p>
              <p>
                {selectedMonth === 'Mar'
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">PDG vs Consumo mensual</CardTitle>
            </CardHeader>
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Ing. Clínica vs Mantenimiento y otros</CardTitle>
            </CardHeader>
            <CardContent className="h-[380px]">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Consumo por cliente interno · {currentMonth.fullMonth}</CardTitle>
            </CardHeader>
                ...
                <>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="rounded-xl border p-3">
                      <div className="font-medium text-slate-900">Ingeniería Clínica</div>
                      <div className="text-slate-600">{fmtMoney(currentClients.ing || 0)}</div>
                    </div>
                    <div className="rounded-xl border p-3">
                      <div className="font-medium text-slate-900">Mantenimiento y otros</div>
                      <div className="text-slate-600">{fmtMoney(currentClients.mant || 0)}</div>
                    </div>
                  </div>
                </>
              ) : (
                <div className="rounded-xl border border-dashed p-6 text-sm text-slate-500">Sin información de clientes internos para este mes.</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">{currentMonth.fullMonth} · ranking de egresos más relevantes</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-3">
                  {currentBreakdown.topItems.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between rounded-xl border p-3">
                      ...
                      <div className="text-right font-semibold">{fmtMoney(item.amount)}</div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="rounded-xl border border-dashed p-6 text-sm text-slate-500">Sin ranking cargado para este mes.</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Qué explicó {currentMonth.fullMonth}</CardTitle>
            </CardHeader>
            <CardContent className="h-[420px]">
              {currentBreakdown?.grouped ? (
                ...
                <div className="flex h-full items-center justify-center rounded-xl border border-dashed p-6 text-center text-sm text-slate-500">Sin agrupación cargada para este mes.</div>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="rounded-2xl shadow-sm">
          <CardHeader>
            <CardTitle className="text-base">Conclusión sugerida para presentar</CardTitle>
          </CardHeader>
